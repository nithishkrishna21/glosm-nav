...
export CLASSES_PATH=${CLASSES_PATH:-glosm_nav/vlm/classes.txt}
...
tmux send-keys -t ${session_name}:0.0 "conda activate vlfm && cd ${VLFM_ROOT} && ${VLFM_PYTHON} -m glosm_nav.vlm.grounding_dino --port ${GROUNDING_DINO_PORT}" C-m
tmux send-keys -t ${session_name}:0.1 "conda activate vlfm && cd ${VLFM_ROOT} && ${VLFM_PYTHON} -m glosm_nav.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "conda activate vlfm && cd ${VLFM_ROOT} && ${VLFM_PYTHON} -m glosm_nav.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "conda activate vlfm && cd ${VLFM_ROOT} && ${VLFM_PYTHON} -m glosm_nav.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
